...
RUN npm install -g ember-cli@1.13.8 && \
    ...
    wget https://bin.equinox.io/c/ekMN3bCZFUn/forego-stable-linux-amd64.tgz && mkdir -p /usr/local/bin/forego && tar -xf forego-stable-linux-amd64.tgz -C /usr/local/bin/forego && rm -f forego-stable-linux-amd64.tgz && \
